...
public class WordLabelList implements ActionListener, MouseListener, PopupMenuListener {
    ...
    /** @param displayPanel The panel that the words' labels should be placed
     *                      in. */
    ...
    public boolean editSelectedWordClue() {
        WordCluePair wcp = m_model.getWordCluePairList().get(m_data.indexOf(m_selectedWordLabel)
                / 2);
        String newClue = JOptionPane.showInputDialog(m_view, "Edit the clue for \""
                                                     + wcp.getWord() + "\":", wcp.getClue());
        if (newClue == null) {
            return false;
        }
        wcp.setClue(newClue);
    ...
    private boolean deleteSelected() {
        ...
        m_model.getWordCluePairList().remove(m_data.indexOf(m_selectedWordLabel)
                / 2);
    ...
    private void doLayout() {
        ...
        int availableWidth = m_displayPanel.getSize().width
                - (m_displayPanel.getInsets().left
                   + m_displayPanel.getInsets().right);
...
//		panelConstraints.setConstraint(SpringLayout.SOUTH, Spring.constant(((modelData.size() / 2) * (ROW_HEIGHT + ROW_GAP)) - ROW_GAP));
         ...
         * word that wasn't covered
         * by the popup menu hangs around because m_displayPanel shrinks and
         * THEN revalidates its area.
